...
function fadeOutLongActiveSound(vol){
	if(vol < 0.05){
		vol = 0;
	}
	else {
	...
	window.GameVariables.LongActiveSound.volume = vol;
	
	if(vol > 0){
		setTimeout(function(){
			fadeOutLongActiveSound(vol);
		}, 50);
	}
	else {
		stopLongActiveSound();
		window.GameVariables.LongActiveSound.volume = 0;
	}
}
